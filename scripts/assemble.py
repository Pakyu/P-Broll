...
def probe_wh(path):
    ...
    if r.returncode != 0 or not r.stdout.strip():
        sys.exit(f"ffprobe 视频尺寸失败: {path}\n{r.stderr[:200]}")
    try:
        w, h = r.stdout.strip().splitlines()[0].split(",")[:2]
    except ValueError:
        sys.exit(f"无法读取视频尺寸: {path}")
    return int(w), int(h)
...
def main():
    ...
    target_total = mf.get("target_duration")
    if target_total is not None:
        try:
            target_total = float(target_total)
        except (TypeError, ValueError):
            sys.exit("target_duration 必须是大于 0 的数字")
        if target_total <= 0:
            sys.exit("target_duration 必须是大于 0 的数字")
    for p in plan:
    ...
    if target_total is not None and abs(total - target_total) > 3:
        print(
            f"  WARN: 预计成片 {total}s 与目标 {target_total:g}s 相差超过 3 秒；"
            "请调整 beat 数、旁白或素材时长",
            file=sys.stderr,
        )
    ...
    for i, p in enumerate(plan):
        ...
        fc.append(
            ...
            f"{pad}trim=duration={p['dur']},setpts=PTS-STARTPTS[p{i}]"
        )

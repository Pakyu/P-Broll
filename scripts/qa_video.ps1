...
# 用法: pwsh qa_video.ps1 <video.mp4> <confirmed-still.png> [out-dir] [bg-hex] [-StripAudio]
param(
...
ffmpeg -y -v error -i $Still -i (Join-Path $OutDir "video-last-frame.jpg") `
    -filter_complex "[0:v]scale=270:480[a];[1:v]scale=270:480[b];[a][b]hstack" `
    (Join-Path $OutDir "end-frame-comparison.jpg")

# 客观分（需要 Pillow；失败不阻塞）
$qaScore = Join-Path $PSScriptRoot "qa_score.py"
$py = "python"
$scoreArgs = @($qaScore, "--video", $Video, "--still", $Still)
if ($BgHex -ne "") { $scoreArgs += @("--bg-hex", $BgHex) }

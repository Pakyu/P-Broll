...
if command -v xxd >/dev/null 2>&1; then
  ok "xxd 可用（macOS/Linux 拼贴首尾帧取色）"
else
  warn "xxd 缺失；低多边形单首帧不受影响，拼贴 prepare_frames.sh 使用前需安装"
fi
...
printf 'INFO  Gate 2 图片路由：Agent 图片能力 → 用户平台 CLI → 小云雀 → HyperFrames（仅拼贴）\n'
printf 'INFO  Gate 2 静帧必须展示并取得明确确认；改图只能重新纯文生图，禁止参考旧图\n'
printf 'INFO  Gate 3 按 frame_mode 使用拼贴首尾帧或低多边形单首帧，不要求固定 API Key\n'

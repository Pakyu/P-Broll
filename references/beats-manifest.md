...
```json
{
  ...
  "defaults": { "theme": "editorial-halftone", "frame_mode": "first-last", "lead": 0.4 },
  "beats": [
    {
      "id": "01-提出问题",
...
- **frame_mode**：`first-last` 表示十套拼贴主题，获准文件是完成态尾帧；
  `single-first` 表示低多边形动画，获准文件是起始首帧。beat 未写时读取
  `defaults.frame_mode`；仍缺省时按 `first-last` 兼容旧项目。
...
- **bg_hex**：仅 `first-last` 由 `prepare_frames.sh` 实测回填；
  `single-first` 不准备空首帧，保持 `null`。
...
- **defaults.theme**（可选）：视觉主题预设英文 ID（见 theme-presets.md；对用户展示
  中文名称，缺省 `editorial-halftone`「编辑半调拼贴」）。**Restyle 玩法**：复制 manifest 换 theme，
  `assemble.py --manifest <变体文件>` 重跑 Gate 2/3——叙事与旁白零改动。
- **defaults.frame_mode**（可选）：应与 theme 对应。十套拼贴主题为
  `first-last`；`low-poly-animation`「低多边形动画」为 `single-first`。
- **vo_duration** 不入库：拼装时 ffprobe 实测，杜绝手抄。

## 生命周期

1. Gate 1：写入 title/target_duration/beats（line/role/caption/theme/frame_mode），计算每条 duration_target/reason，并核对全片总时长预算
2. Gate 2：按图片生成路由执行；拼贴生成完成态尾帧，低多边形生成起始首帧；展示实际静帧并取得明确确认后回填 gate2 approval 字段
3. Gate 3：核验 gate2 approval 与当前模式的获准文件一致；拼贴准备首尾帧，低多边形直接使用单首帧；确认模型和费用后回填 duration_submitted、generation_route/platform/model 与 video
